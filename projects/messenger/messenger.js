...
async function main(){
    ...
    if(process.argv[3] == "deploy"){
        if(network.type == "evm"){
            ...
            exec(
                `cd chains/evm && forge build && forge create --legacy --constructor-args ${network.bridgeAddress} --rpc-url ${network.rpc} --private-key ${network.privateKey} src/Messenger.sol:Messenger && exit`,
                ((err, out, errStr) => {
                    if(err){
                        throw new Error(err);
                    } 
        ...
        if(network.type == "evm"){
            ...
            const tx = await (await messenger.sendMsg(Buffer.from(process.argv[4]), {gasLimit: 100000, gasPrice: 250000000000})).wait();
        ...
        if(network.type == "evm"){
            ...
            const tx = await messenger.receiveEncodedMsg(Buffer.from(vaaBytes, "base64"), {gasLimit: 100000, gasPrice: 250000000000});
